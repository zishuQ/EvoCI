...
class FakeDocker:
    ...
    def run(self, args: list[str], *, timeout: float) -> subprocess.CompletedProcess[str]:
        ...
        if args[:1] == ["cp"]:
            ...
            self.copied.append(dest.split(":", 1)[-1])
    ...
    def _exec(self, args: list[str], command: list[str]) -> subprocess.CompletedProcess[str]:
        ...
        has_candidate = "/tmp/candidate.patch" in self.copied
